...
const Wizard = forwardRef((props, ref) => {
  // ** Props
  const { type, className, contentClassName, headerClassName, steps, separator, options, instance, defaultStepperClass, disabled } = props
      ...
      return (
        <Fragment key={step.id}>
          ...
          <div
            ...
            data-target={`#${step.id}`}
          >
            <button type='button' className='step-trigger' disabled={disabled} >
  ...
  return (
    ...
    >
      ...
      <div className={defaultStepperClass ? "" : "bs-stepper-content"}>{renderContent()}</div>
    </div>
  )
})
